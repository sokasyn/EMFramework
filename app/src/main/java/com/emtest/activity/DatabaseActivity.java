...
//                createDatabaseWithEMLib();
                ...
                createDatabaseWithEMLib2();
                Log.i(TAG,"= = = = = = = = = = 新版本 create table");
                try{
                    createTablesV2();
                }catch (JSONException e){
                    e.printStackTrace();
...
//                SQLiteDatabase db2 = dbManager.getDatabase();
    ...
    // = = = = = = = = = = = = = 新版本 = = = = = = = = = = = = =
    private void createDatabaseWithEMLib2(){
        Log.i(TAG,"createDatabaseWithEMLib2");
        // 构造配置模版
        com.emin.digit.mobile.android.database.EMDaoConfig daoConfig = new com.emin.digit.mobile.android.database.EMDaoConfig();
        daoConfig.setContext(Number1Activity.this);
        daoConfig.setDbName("number1_4.db");
        daoConfig.setDbVersion(1);
        daoConfig.setTargetDirectory(null);
//        EMSQLiteDatabase database = EMSQLiteDatabase.create(daoConfig);
        com.emin.digit.mobile.android.database.EMDatabaseManager.getInstance().createOrOpenDatabase(daoConfig);
    }


    private void createTablesV2() throws JSONException{
        Log.i(TAG,"Test createTables");
        JSONObject tableJson = new JSONObject();
        // table:user
        tableJson.put("USER2","name,age");

        // table:account
        tableJson.put("ACCOUNT2","name,password");

        // table:address
        tableJson.put("ADDRESS2","id,pid,name");

        String jsonStr = tableJson.toString();
        Log.i(TAG,"json string:" + jsonStr);

//        EMDatabaseManager.getInstance().createTables(jsonStr);
        com.emin.digit.mobile.android.database.EMDatabaseManager.getInstance().createTables(jsonStr);
    }
}
